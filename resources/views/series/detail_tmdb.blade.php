        ...
        <section class="detail_movie">
            <div class="box_poster_detail">
                ...
                @if (!empty($serie_data->backdrop_path))
                    <img 
                        src="https://image.tmdb.org/t/p/w500/{{ $serie_data->backdrop_path }}" 
                        alt="{{ $serie_data->name }}">
                @endif
            </div>
            ...
            <div class="infos">
                <div class="header_infos">
                    @if (!empty($serie_data->poster_path))
                        <img 
                            width="200px" 
                            src="https://image.tmdb.org/t/p/w500/{{ $serie_data->poster_path }}" 
                            alt="{{ $serie_data->name }}" 
                            id="poster_detail">
                    @endif
                    ...
                    <div class="details_infos">
                        <div class="about">
                            ...
                            <ul>
                                ...
                                <li><strong>Note moyenne :</strong> {{ $serie_data->vote_average }} / 10 ({{ $serie_data->vote_count }} votes)</li>
                            </ul>
                        </div>
                            ...
                            <ul>
                                @if (!empty($serie_data->credits->cast) && count($serie_data->credits->cast) > 0)
                                    @for ($i = 0; $i < min(8, count($serie_data->credits->cast)); $i++)
                                        <li><img src="https://image.tmdb.org/t/p/w500/{{ $serie_data->credits->cast[$i]->profile_path }}" alt=""><strong>{{ $serie_data->credits->cast[$i]->name }}</strong></li>
                                    @endfor
